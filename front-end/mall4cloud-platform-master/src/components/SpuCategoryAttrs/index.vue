    ...
    <div class="part-section">
      <div v-for="(attrs, attrIdx) in attrsList" :key="attrIdx" class="part-sec-item">
        <div class="flx">
          ...
          <el-select v-if="attrs.attrValues.length" v-model="attrs.attrValue" placeholder="请选择" @change="getValueOfBasicAttrs(attrs, 0)" style="width:230px">
            <el-option
          ...
          <el-input :readonly="true" v-else v-model="attrs.attrValue" @blur="getValueOfBasicAttrs(attrs, 1)" style="width:230px" />
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    getValueOfBasicAttrs(attrs, st) {
      this.$emit('getValueOfBasicAttrs', attrs, st)
    }
  }
}
</script>
